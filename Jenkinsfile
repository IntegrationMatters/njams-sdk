...
node('master') {
    ...
    stage('Build SDK') {
        ...
        dir('njams-sdk') {
            try {
                sh "'${mvnHome}/bin/mvn' clean ${buildGoal}   -Pjenkins-cli -DrevisionNumberPlugin.revision=${env.BUILD_NUMBER} -DscmBranch=${scmInfo.GIT_BRANCH} -DscmCommit=${scmInfo.GIT_COMMIT}"
            } finally {

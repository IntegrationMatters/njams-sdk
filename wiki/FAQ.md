...
`ClientSettings` is a key/value view over your configuration (transport selection, flush tuning, data masking,
Argos, …); the available keys are listed in the tables further down.
...
When configuration comes from several places — a config file, system properties, environment variables, built-in
defaults — use `HierarchicalSettings`. It is a `ClientSettings` that consults an ordered list of named layers and, for
each key, returns the value from the **first** layer that defines it. Writes are applied only to the base (first) layer.

Build it with the fluent builder. List layers from highest priority to lowest, with defaults last so they only apply
when no other layer provides the key:
...
- Start the chain with `from(ClientSettings)`, `from(Map)`, `from(Properties)`, or `fromEmpty()` for an in-memory base.
  The base layer is the only one that receives writes.
- Add further layers with `andThen(...)` (a `ClientSettings`, `Map`, or `Properties`), `andThenSystemProperties()`, or
  `andThenEnvironmentVariables()`.
- System-property and environment layers can be narrowed with `withPrefixFilter(...)` and/or `withRegexFilter(...)` (
  combined with AND); filters match the actual storage key (the system-property or environment-variable name).
- Earlier layers win, so list your most specific overrides first and the defaults layer last as a fallback.

**Environment variables.** The environment layer resolves the SDK's dotted property keys from their upper-snake-case
environment equivalents — e.g. reading `njams.sdk.communication` looks up the environment variable
...
| Property                                         | Default    | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Tags                                                                                                                                                                                                                                                                                 |
|--------------------------------------------------|------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `njams.client.sdk.discardpolicy`                 | `none`     | Replaced by `njams.sdk.discardpolicy`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | <kbd style="background-color:#cf222e;color:#fff;border-color:#cf222e">deprecated 5.0.0</kbd><br><kbd style="background-color:#24292f;color:#fff;border-color:#24292f">removed 6.0.0</kbd>                                                                                            |
| `njams.client.sdk.disable.secure.processing`     | `false`    | Replaced by `njams.sdk.disable.secure.processing`.                                                                                                                                                                                                                                                                                                                                                                                                                                                               | <kbd style="background-color:#2da44e;color:#fff;border-color:#2da44e">since 4.2.0</kbd><br><kbd style="background-color:#cf222e;color:#fff;border-color:#cf222e">deprecated 5.0.0</kbd><br><kbd style="background-color:#24292f;color:#fff;border-color:#24292f">removed 6.0.0</kbd> |
| `njams.client.sdk.disable.startdata`             | `false`    | Replaced by `njams.sdk.disable.startdata`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | <kbd style="background-color:#2da44e;color:#fff;border-color:#2da44e">since 4.2.3</kbd><br><kbd style="background-color:#cf222e;color:#fff;border-color:#cf222e">deprecated 5.0.0</kbd><br><kbd style="background-color:#24292f;color:#fff;border-color:#24292f">removed 6.0.0</kbd> |
| `njams.client.sdk.flush_interval`                | `30`       | Replaced by `njams.sdk.flush_interval`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | <kbd style="background-color:#cf222e;color:#fff;border-color:#cf222e">deprecated 5.0.0</kbd><br><kbd style="background-color:#24292f;color:#fff;border-color:#24292f">removed 6.0.0</kbd>                                                                                            |
| `njams.client.sdk.flushsize`                     | `5242880`  | Replaced by `njams.sdk.flushsize`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | <kbd style="background-color:#cf222e;color:#fff;border-color:#cf222e">deprecated 5.0.0</kbd><br><kbd style="background-color:#24292f;color:#fff;border-color:#24292f">removed 6.0.0</kbd>                                                                                            |
| `njams.client.sdk.maxqueuelength`                | `8`        | Replaced by `njams.sdk.maxqueuelength`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | <kbd style="background-color:#2da44e;color:#fff;border-color:#2da44e">since 4.1.0</kbd><br><kbd style="background-color:#cf222e;color:#fff;border-color:#cf222e">deprecated 5.0.0</kbd><br><kbd style="background-color:#24292f;color:#fff;border-color:#24292f">removed 6.0.0</kbd> |
| `njams.client.sdk.maxsenderthreads`              | `8`        | Replaced by `njams.sdk.maxsenderthreads`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | <kbd style="background-color:#2da44e;color:#fff;border-color:#2da44e">since 4.1.0</kbd><br><kbd style="background-color:#cf222e;color:#fff;border-color:#cf222e">deprecated 5.0.0</kbd><br><kbd style="background-color:#24292f;color:#fff;border-color:#24292f">removed 6.0.0</kbd> |
| `njams.client.sdk.minsenderthreads`              | `1`        | Replaced by `njams.sdk.minsenderthreads`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | <kbd style="background-color:#2da44e;color:#fff;border-color:#2da44e">since 4.1.0</kbd><br><kbd style="background-color:#cf222e;color:#fff;border-color:#cf222e">deprecated 5.0.0</kbd><br><kbd style="background-color:#24292f;color:#fff;border-color:#24292f">removed 6.0.0</kbd> |
| `njams.client.sdk.senderthreadidletime`          | `10000`    | Replaced by `njams.sdk.senderthreadidletime`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | <kbd style="background-color:#2da44e;color:#fff;border-color:#2da44e">since 4.0.4</kbd><br><kbd style="background-color:#cf222e;color:#fff;border-color:#cf222e">deprecated 5.0.0</kbd><br><kbd style="background-color:#24292f;color:#fff;border-color:#24292f">removed 6.0.0</kbd> |
...
| `njams.sdk.debug.messagedir`                     | —          | Directory path for debug message dumps. When set, each outbound message is written as an individual `.json` file to a per-run subdirectory (`yyyyMMdd-HHmmss`) under this path. Each file contains the message's logical headers and JSON body. **For development/debug use only.** When not set the feature is completely inactive.                                                                                                                                                                             | <kbd style="background-color:#2da44e;color:#fff;border-color:#2da44e">since 6.0.0</kbd>                                                                                                                                                                                              |
...
| `njams.sdk.disable.startdata`                    | `false`    | When `true`, globally disables collecting job start-data. Also disables replay functionality. Use `njams.sdk.bootstrap.recording` instead. Recording is now fully server-driven.                                                                                                                                                                                                                                                                                                                                 | <kbd style="background-color:#2da44e;color:#fff;border-color:#2da44e">since 5.0.0</kbd><br><kbd style="background-color:#cf222e;color:#fff;border-color:#cf222e">deprecated 5.0.0</kbd><br><kbd style="background-color:#24292f;color:#fff;border-color:#24292f">removed 6.0.0</kbd> |
| `njams.sdk.discardpolicy`                        | `discard`  | What to do with a message that cannot be sent immediately (a momentarily full destination or a broken connection). Values: `none`, `onconnectionloss`, `discard`. Behaves identically across all transports (HTTP, JMS, Kafka) — see [What does njams.sdk.discardpolicy control](#what-does-njamssdkdiscardpolicy-control). **⚠ Default changed from `none` to `discard` in 6.0.0.** Clients upgrading from earlier versions will now discard undeliverable messages instead of blocking; set to `none` explicitly to restore the previous blocking behavior.                                                                                                                                     | <kbd style="background-color:#e36209;color:#fff;border-color:#e36209">changed 6.0.0</kbd>                                                                                                                                                                                            |
| `njams.sdk.flush_interval`                       | `30`       | Flush interval in seconds. The current log message is flushed if no message has been sent within this interval. See [How does the SDK control sending messages to nJAMS Server](#how-does-the-sdk-control-sending-messages-to-njams-server).                                                                                                                                                                                                                                                                   |                                                                                                                                                                                                                                                                                      |
| `njams.sdk.flushsize`                            | `5242880`  | Approximate maximum log message body size in bytes. The message is flushed when the estimated size exceeds this threshold. The size estimate accounts for every activity's base size, event messages, event codes and attributes, in addition to payloads, stack traces and start data. See [How does the SDK control sending messages to nJAMS Server](#how-does-the-sdk-control-sending-messages-to-njams-server).                                                                                            |                                                                                                                                                                                                                                                                                      |
...
| `njams.sdk.payload.limit.applyToStartData`       | `false`    | When `true`, the payload limit (`njams.sdk.payload.limit.mode` / `njams.sdk.payload.limit.size`) is also applied to a job's **start data**. Start data that is truncated or discarded this way makes that job **non-replayable** (`$njams_recorded` is cleared); start data that fits the limit keeps the job replayable. Default `false` leaves start data unlimited. Note: to actually bound memory while serializing very large start data, register a payload-size-honouring serializer (e.g. the JSON serializer) for the start-data type — the default `toString`-based serializer builds the full representation first, so this option then only bounds the transmitted/stored size. | <kbd style="background-color:#2da44e;color:#fff;border-color:#2da44e">since 6.0.0</kbd> |
...
| `njams.sdk.process.exclude.regex.<name>`         | —          | Excludes processes whose path matches the given Java regular expression. Append any name after the prefix to define multiple patterns. Matching is a full match against the canonical process path (e.g. `>MyApp>MyProcess>`) and is case-sensitive — prefix with `(?i)` for case-insensitive matching. Applied **in addition** to the process filters configured via the nJAMS server: a process is excluded if it is excluded by either mechanism. An explicit include filter takes precedence and re-includes it. Invalid expressions are ignored. | <kbd style="background-color:#2da44e;color:#fff;border-color:#2da44e">since 6.0.0</kbd>                                                                                                                                                                                              |
| `njams.sdk.senderthreadidletime`                 | `10000`    | Time in milliseconds after which an idle non-core sender thread is removed.                                                                                                                                                                                                                                                                                                                                                                                                                                      |                                                                                                                                                                                                                                                                                      |
| `njams.sdk.server.compatibility`                 | —          | Server version compatibility switch. Set to the version of the nJAMS server you are running against (e.g., `6.1`) to restore SDK behaviors that were changed in 6.0 but are not yet supported by that server version. Leave unset for current SDK behavior. **Effects when set to `6.1`:** transitions created without an explicit name carry the transition ID as their name (required for nJAMS server versions that do not yet support nameless transitions). Remove this setting once the server is updated. | <kbd style="background-color:#2da44e;color:#fff;border-color:#2da44e">since 6.0.0</kbd><br><kbd style="background-color:#cf222e;color:#fff;border-color:#cf222e">deprecated 6.0.0</kbd>                                                                                              |
...
| `njams.sdk.communication.http.dataprovider.prefix`                    | —       | Replaced by `njams.sdk.communication.http.dataprovider.suffix`.                                                                                                                         | <kbd style="background-color:#cf222e;color:#fff;border-color:#cf222e">deprecated 5.0.0</kbd><br><kbd style="background-color:#24292f;color:#fff;border-color:#24292f">removed 6.0.0</kbd>                                                                                            |
...
| `njams.sdk.communication.jms.destination`                      | —            | Common prefix for the event, command, and error queues / topics (e.g. `njams`). `njams.sdk.communication.jms.destination.prefix` is an equally valid alternative key (see below); neither is deprecated.                            |                                                                                         |
| `njams.sdk.communication.jms.destination.prefix`               | —            | Equally valid alternative to `njams.sdk.communication.jms.destination`, which is also the prefix of `njams.sdk.communication.jms.destination.commands` and therefore cannot be expressed in a hierarchical configuration format such as YAML. Use this key instead in such sources.                                                | <kbd style="background-color:#2da44e;color:#fff;border-color:#2da44e">since 6.0.0</kbd> |
...
## How does the SDK control sending messages to nJAMS Server

nJAMS server, respectively Elasticsearch, is not very good at handling high-frequency updates to the
same job/`logId`, even though it handles a high volume of log messages for different jobs in parallel
without issue. Because of this, the SDK — not the client application — decides when a job's log message
is actually sent:

- **Project messages** are sent once when `Njams` is started, plus any additional incremental project
  messages a client explicitly sends for supplemental resources. The nJAMS server can also request a
  resend; the SDK responds with the current, full merged project message.
- **Log messages** are identified by `logId` — one per job (process execution). Sending a second message
  with the same `logId` updates that job's message on the server rather than creating a new one.
- For a typical short-running job, the SDK sends **exactly one** log message, at job end.
- For a long-running or large job, the SDK sends an **intermediate** log message only when the job's
  accumulated data exceeds `njams.sdk.flushsize` or has not been flushed within
...
## What does njams.sdk.discardpolicy control

When a send attempt does not succeed immediately — the destination is momentarily full, or the connection is
broken — `njams.sdk.discardpolicy` decides what happens to the message. All three built-in transports (HTTP, JMS,
...
- `discard` (default since 6.0.0): never delays the monitored application. A send that does not succeed
  immediately is given up on at once and the message is dropped, whether the underlying cause is a full
  destination or a broken connection.

See [What happens when the communication backend is unreachable at startup](#what-happens-when-the-communication-backend-is-unreachable-at-startup)
for how this policy applies to messages produced while the initial connection is still being established.
...
nJAMS renders each process as an SVG diagram shown in the server UI. Producing that diagram is split into two
independent steps:
...
that computes group dimensions from their contents, followed by a top-down placement pass using per-column widths — and
correctly handles parallel branches, convergence nodes, and nested groups. For a detailed description of the algorithm
and its known limitations, see [CommonBfsModelLayouter Algorithm](CommonBfsModelLayouter-Algorithm).
...
is needed — `CommonBfsModelLayouter` is now used automatically. However, your model may look different due to the
improved layout.

**Custom layouter** — the standard way to control layout is to implement
`com.im.njams.sdk.model.layout.ProcessModelLayouter`. The interface has a single method:
...
example, when coordinates are imported from an external source — use `NoopLayouter` instead to skip the layout step
entirely:
...
The built-in `NjamsProcessDiagramFactory` generates SVG that meets the structural and formatting requirements of the
nJAMS server. **Replacing it with a completely custom `ProcessDiagramFactory` implementation is not recommended** — the
server relies on specific SVG conventions that `NjamsProcessDiagramFactory` encodes. Instead, extend or configure the
existing factory.

**XSLT post-processing** — supply an XSLT stylesheet to be applied to the SVG DOM just before it is serialized to a
string. Use this to restyle, reorder, or augment the generated SVG without touching the drawing logic:
...
**Extra elements hook** — subclass `NjamsProcessDiagramFactory` and override the empty
`drawExtraElements(NjamsProcessDiagramContext)` method to append additional SVG elements after all activities,
transitions, and groups have been drawn:

```java
public class MyDiagramFactory extends NjamsProcessDiagramFactory {
    public MyDiagramFactory(Njams njams) {
        super(njams);
    }
...
2. The output will show something like:
   ...
   > **Note:** Do not prepend `??` to the value you want to encode — the `??` prefix is the marker the SDK uses to
   detect encoded values.

3. Replace the corresponding value in your configuration file with the encoded value. The SDK decodes it automatically
   when needed.

## Does the client path need to be unique

Yes. The client path uniquely identifies a `Njams` instance in the nJAMS infrastructure. The server and the SDK's
communication layer use it as an address to route commands back to the correct client. Every `Njams` instance must be
initialized with a path that is unique across all connected clients.
...
1. Stop the client.
2. Open the JSON file used for dynamic configuration (tracepoints, log modes, etc.). By default this is
   `configuration.json`. The path can be changed via `njams.sdk.configuration.file.file`.
...
> **Note:** If the client has not been started yet or no server-side configuration changes have been made, the file may
> not exist. Create it manually with this minimum content:
> ```json
> {
>   "logMode": "COMPLETE",
>   "processes": {},
...
The Argos feature allows the SDK to send JVM and custom application metrics to an nJAMS Agent. After `Njams.start()`,
the Argos sender starts automatically and transmits collected metrics every 10 seconds. Configure the agent connection
via the settings properties (see [Argos / Metrics](#argos--metrics) above).

By default, the Argos sender is enabled but no collectors are registered. The SDK ships with a built-in `JVMCollector`
for standard JVM metrics. Register it before or after starting the `Njams` instance:
...
`ActivityModel` has a `mapping` attribute that is displayed in the nJAMS UI when you click on an activity. The content
can take three forms; the UI selects the appropriate viewer automatically:
...
To trigger the tree viewer, the root object must have a `name` (the root node label) and an `entries` array. Each entry
is either a **leaf** — carrying a `name` and a `value` — or a **branch** — carrying a `name` and its own nested`entries`
array:
...
The following example maps the structure of an XSLT stylesheet, where the `entries` hierarchy mirrors the XML element
nesting:
...
global variable reference for the destination queue:

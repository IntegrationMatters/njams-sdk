...
| `njams.sdk.flushsize`                            | `5242880`  | Minimum size of a log message before it is flushed. The message is flushed when the estimated size exceeds this threshold. The size estimate accounts for every activity's base size, event messages, event codes and attributes, in addition to payloads, stack traces and start data. See [How does the SDK control sending messages to nJAMS Server](#how-does-the-sdk-control-sending-messages-to-njams-server).                                                                                            |                                                                                                                                                                                                                                                                                      |
...
The settings below are debug only — intended for development and troubleshooting, not for production use.

| Property                      | Default | Description                                                                                                                                                                                                                                       | Tags                                                                                      |
|--------------------------------|---------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------|
| `njams.sdk.debug.messagedir`  | —       | Directory path for debug message dumps. When set, each outbound message is written as an individual `.json` file to a per-run subdirectory (`yyyyMMdd-HHmmss`) under this path. Each file contains the message's logical headers and JSON body. When not set the feature is completely inactive. | <kbd style="background-color:#2da44e;color:#fff;border-color:#2da44e">since 6.0.0</kbd> |

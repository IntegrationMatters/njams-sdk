...
| `njams.sdk.flushsize` | `5242880` | Approximate maximum log message body size in bytes. The message is flushed when the estimated size exceeds this threshold. **The size estimate now accounts for every activity's base size, event messages, event codes and attributes, in addition to payloads, stack traces and start data.** Previously these were omitted, so jobs that accumulated many activities could grow well beyond this threshold before being sent on the flush interval. The estimate remains a conservative (slightly low) approximation of the serialized wire size; the server re-derives the authoritative size on receipt. | <kbd style="background-color:#e36209;color:#fff;border-color:#e36209">changed 6.0.0</kbd> |

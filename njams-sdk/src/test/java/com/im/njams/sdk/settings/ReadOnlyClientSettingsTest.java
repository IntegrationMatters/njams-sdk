...
/**
 * Covers the default methods on {@link ReadOnlyClientSettings} / {@link ClientSettings} and the
 ...
 */
public class ReadOnlyClientSettingsTest {
    ...
    public void fromMap_readOnlyFactoryReturnsReadable() {
        backing.put("a", "1");
        ReadOnlyClientSettings ro = ReadOnlyClientSettings.from(backing);
    ...
    public void fromEnvironment_publicFactoryWrapsSystemEnv() {
        ReadOnlyClientSettings ro = ReadOnlyClientSettings.fromEnvironment(key -> true);
        int count = 0;
    ...
    public void fromEnvironment_publicFactoryAcceptsNullFilter() {
        ReadOnlyClientSettings ro = ReadOnlyClientSettings.fromEnvironment(null);
        int count = 0;
        for (Entry<String, String> e : ro) {
            count++;
        }
    ...
    public void environment_filterAppliesToGetProperty() {
        ...
        ReadOnlyFilteringSettings ro =
            new ReadOnlyFilteringSettings(ReadOnlyClientSettings.from(env), key -> key.startsWith("F"));
    ...
    public void environment_filterAppliesToIteration() {
        ...
        env.put("BAZ", "3");
        ReadOnlyFilteringSettings ro =
            new ReadOnlyFilteringSettings(ReadOnlyClientSettings.from(env), key -> key.startsWith("B"));
    ...
    public void environment_filterAppliesToTypedGetters() {
        ...
        ReadOnlyFilteringSettings ro =
            new ReadOnlyFilteringSettings(ReadOnlyClientSettings.from(env), key -> key.startsWith("ALLOWED"));
    ...
    public void environment_acceptAllFilterExposesAllEntries() {
        ...
        env.put("FOO", "1");
        env.put("BAR", "2");
        ReadOnlyFilteringSettings ro = new ReadOnlyFilteringSettings(ReadOnlyClientSettings.from(env), key -> true);
        assertEquals("1", ro.getProperty("FOO"));
    ...
    public void filteringSettings_nullFilterInConstructorAcceptsAllKeys() {
        ...
        backing.put("FOO", "1");
        backing.put("BAR", "2");
        ReadOnlyFilteringSettings ro = new ReadOnlyFilteringSettings(ReadOnlyClientSettings.from(backing), null);
        assertEquals("1", ro.getProperty("FOO"));
        assertEquals("2", ro.getProperty("BAR"));
        assertTrue(ro.containsKey("FOO"));
        assertTrue(ro.containsKey("BAR"));
    ...
    @Test
    public void fromSystemProperties_misbehavingPropertiesSubclass_isConsistent() {
        // Symmetric to fromProperties_misbehavingSubclass_allReadsAgree: install a Properties
        // subclass whose real storage is exposed only via the string API. Replacing
        // System.getProperties() with such a subclass would silently break the SDK if the
        // filtering layer still used the inherited Hashtable Map view.
        Properties original = System.getProperties();
        Properties custom = new Properties() {
            private final Map<String, String> store = new HashMap<>(Map.of("njams.test.camel.k", "v"));
        ...
        try {
            ...
            List<Entry<String, String>> entries = new ArrayList<>();
            ws.forEach(entries::add);
            assertTrue(entries.stream().anyMatch(e -> "njams.test.camel.k".equals(e.getKey())));
        } finally {
    ...
    @Test
    public void filteringDecoratesSettings_innerWriteVisibleThroughFilter() {
        // Filtering is now a decorator: writes through the inner settings (not the wrapper)
        ...
        inner.put("a.x", "1");
        inner.put("b.y", "2");
        assertEquals("1", ws.getProperty("a.x"));
        assertNull(ws.getProperty("b.y"));
        assertTrue(ws.containsKey("a.x"));
        assertFalse(ws.containsKey("b.y"));
    }
    ...
    public void keyTransformer_appliedOnGetProperty() {
        ...
        ReadOnlyFilteringSettings ro = new ReadOnlyFilteringSettings(
            ReadOnlyClientSettings.from(backing), null, ReadOnlyFilteringSettings.ENVIRONMENT_KEY_TRANSFORMER);
    ...
    public void keyTransformer_filterAppliesToTransformedKey() {
        ...
        ReadOnlyFilteringSettings ro = new ReadOnlyFilteringSettings(
            ReadOnlyClientSettings.from(backing), key -> key.startsWith("HELLO"),
    ...
    public void keyTransformer_nullActsAsIdentity() {
        ...
        ReadOnlyFilteringSettings ro = new ReadOnlyFilteringSettings(ReadOnlyClientSettings.from(backing), null, null);
        assertEquals("1", ro.getProperty("Hello.world"));
    ...
    public void keyTransformer_cachesTransformedKeys() {
        ...
        ReadOnlyFilteringSettings ro = new ReadOnlyFilteringSettings(ReadOnlyClientSettings.from(backing), null, counting);
    ...
    public void keySet_filteringSettings_appliesFilter() {
        ...
        backing.put("BAR", "2");
        backing.put("BAZ", "3");
        ReadOnlyFilteringSettings ro = new ReadOnlyFilteringSettings(ReadOnlyClientSettings.from(backing), key -> key.startsWith("B"));
    ...
    public void keySet_filteringSettings_emptyWhenFilterRejectsAll() {
        Map<String, String> backing = new HashMap<>();
        backing.put("FOO", "1");
        ReadOnlyFilteringSettings ro = new ReadOnlyFilteringSettings(ReadOnlyClientSettings.from(backing), key -> false);

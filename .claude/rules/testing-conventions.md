...
Tests use JUnit 4 + Mockito. The `AbstractTest` base class provides common test setup. Transport-specific tests (JMS) may require mocking of connection infrastructure. Kafka and Argos/JVM metrics tests are out of scope — see `kafka-argos-deprecated.md`.
...
Cover all new/changed code with tests. For modifications to existing code, invoke the `njams-safe-modification` skill FIRST to establish baseline coverage (see `development-workflow-skills.md`).

## Bug fixes

**Existing test cases must never be modified without explicit user permission** — if a fix causes a test to fail, the fix is wrong. If there is a genuine reason to believe a test is incorrect, explain the specific conflict and ask the user before changing anything. See `development-workflow-skills.md` for the `njams-bug-fix` skill this workflow requires.

When code relocates without behavior change, move its tests as-is with identical assertions — that parity is the proof the move preserved behavior.
